...
class Algorithm():
    """
    Runs the desired algorithm upon creation, uses flags to keep track of which one to run.
    """

    def __init__(self, world, start, goal, flag): # Mostly same as previous assignment, will comment alterations
        """
    ...
    def run(self):
        """
        Runs any algorithm, adopted more or less line by line from the pseudocode in the supplement.
        Only changes from GridsWithObstacles.py are commented.
        """
        while True:
            if len(self.open) == 0:
                self.mark()
    ...
    def h(self, node): # Estimate using straight line to goal
        h = math.sqrt((node.pos[0] - self.goal.pos[0])**2 + (node.pos[1] - self.goal.pos[1])**2) * 2
        return h

    def getChildren(self, node):
        """
        Generates and appends children if valid position. Uses the boundaries of the matrix as limit.
        Alternatively this could all be done during world building, but would then do redundant work on nodes that are never utilized.
        """
        posX = node.pos[0]
        posY = node.pos[1]
        if (posY - 1) >= 0:
...
gui = image.gui(Algorithm(world, start, goal, 'A').getParams(), len(world[0]), len(world))
map = gui.send_it()
map.show()
